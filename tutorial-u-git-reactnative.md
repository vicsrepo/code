...
const { download\_url } = route.params;
...
renderItem={({ item }) => (\
\<TouchableOpacity\
onPress={() => navigation.navigate('MarkdownViewer', { download\_url: item.download\_url })}\
...
const fetchFileContent = async () => {\
try {\
const response = await fetch(download\_url);\
const text = await response.text();\
setContent(text);\
} catch (error) {\
console.error('Chyba při načítání souboru:', error);\
...
const GITHUB\_API = 'https://api.github.com/repos/vics-cloud/docs/contents/docs';
...
const res = await fetch(GITHUB\_API);\
const data = await res.json();\
setTree(data);\
};\
fetchRoot();\
...
{selectedFile ? (\
\<MarkdownViewer route=\{{ params: { download\_url: selectedFile.download\_url } \}} />\
) : null}\
);\
};
...
\<Text style=\{{ color: isDark ? '#fff' : '#000' \}}>Téma\
